...
namespace fft
{
    ...
    // 迭代实现
    inline void dif_iter(Complex inout[], size_t fft_len)
    {
        if (fft_len <= 1)
        {
            return;
        ...
        // 对每一层进行FFT计算
        for (size_t rank = fft_len; rank >= 2; rank /= 2)
        {
            ...
            for (size_t k = 0; k < fft_len; k += rank)
            {
                ...
                {
                    auto x0 = inout[k + i], x1 = inout[k + i + stride];
                    inout[k + i] = x0 + x1;
                    inout[k + i + stride] = (x0 - x1) * omega_ptr[i];
                }
            }
        }
    }
...
inline void dif(Complex inout[], size_t fft_len)
{
    ...
    inline void fft_dif(Complex inout[], size_t fft_len)
    {
        dif(inout, fft_len);
